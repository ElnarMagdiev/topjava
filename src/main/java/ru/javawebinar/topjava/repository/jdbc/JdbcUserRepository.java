...
//        return jdbcTemplate.queryForObject("SELECT * FROM users WHERE email=?", ROW_MAPPER, email);
    ...
    private Map<Integer, Set<Role>> getAllRoles() {
        Map<Integer, Set<Role>> rolesWithUserId = new HashMap<>();
        jdbcTemplate.query("""
                    SELECT * FROM user_role ur
                """, rs -> {
            rolesWithUserId.computeIfAbsent(rs.getInt("user_id"), userId -> EnumSet.noneOf(Role.class))
                    .add(Role.valueOf(rs.getString("role")));
        });
        return rolesWithUserId;
    }

    private List<Role> getRoles(User user) {
        return jdbcTemplate.queryForList("""
                    SELECT ur.role FROM user_role ur
                    WHERE ur.user_id = ?
